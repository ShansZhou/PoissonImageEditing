img_source = double(imread('image/apple.jpg'));
img_target = double(imread('image/heart.jpg'));

img_target = imresize(img_target,0.3);
...
%% fixed cropping
target_col = [1, 1, 65, 65];
target_row = [1, 67, 67, 1];
offset_col = 200;
offset_row = 100;
...
imshow(img_source/255),title('point a position to paste target image');
line(source_col,source_row);

mask_target = roipoly(img_target/255,target_col,target_row);
mask_source = roipoly(img_source/255,source_col,source_row);

maskSource_value = img_source .* mask_source;
maskTarget_value = img_target .* mask_target;
...
smallTargetMask_value = img_target .* small_targetMask;
...
for n =1:size(fDestinationBorder_index,1);
    fDestinationBorder_value(indexOfBorder_target(n,1),indexOfBorder_target(n,2)) =...
        img_source(fDestinationBorder_index(n,1),fDestinationBorder_index(n,2));
end
[maskTarget_row, maskTarget_col] = find(mask_target);
f_destination = zeros(size(img_target));
...
result_directCloning = img_source;
...
for n =1:size(smallTargetMask_row)
    result_directCloning(int16(smallTargetMask_row(n)+offset(2)),int16(smallTargetMask_col(n)+offset(1))) =...
        img_target(int16(smallTargetMask_row(n)),int16(smallTargetMask_col(n)));
end
% figure;imshow(result_directCloning/255);title('direct cloning')


%% caluculate importing gradients Vpq_importing
Vpq_importing  = zeros(size(img_target));
[maskTarget_row, maskTarget_col] = find(mask_target);
for n =1:size(maskTarget_row)
...
result_importingGradient = img_source;
for n =1:size(smallTargetMask_row);
    result_importingGradient(int16(smallTargetMask_row(n)+offset(2)),int16(smallTargetMask_col(n)+offset(1))) = f_importing(n);
end
figure,imshow(result_importingGradient/255);title('importing gradient')

%% calculate mixing gradients Vpq_mixing
Vpq_mixing = zeros(size(img_target));
[maskSource_row, maskSource_col] = find(mask_source);
for n =1:size(maskTarget_row)
%     maxRow_t = maskTarget_row(n)+1;
%     maxCol_t = maskTarget_col(n)+1;
%     if maskTarget_row(n)+1 > size(maskTarget_value,1)
%         maxRow_t = maskTarget_row(n);
%     elseif maskTarget_col(n)+1 > size(maskTarget_value,2)
%             maxCol_t = maskTarget_col(n);
%     end
...
%     maxRow_s = maskSource_row(n)+1;
%     maxCol_s = maskSource_col(n)+1;
%     if maskSource_row(n)+1 > size(maskTarget_value,1)
%         maxRow_s = maskSource_row(n);
%     elseif maskSource_col(n)+1 > size(maskTarget_value,2)
%             maxCol_s = maskSource_col(n);
%     end
